...
1. 說一句中文（例：「今天天氣很好」）
2. **拉長音發「ㄙ～～」或噓聲約 2 秒**，音量正常、靠近麥克風

第二步是關鍵。「ㄙ」含大量高頻，而 8 kHz 窄頻連結**不可能**傳遞 4 kHz 以上的能量。

#### 判準：看「高頻帶上升量」而不是看「高頻有沒有能量」

| 判定 | 意義 |
|---|---|
| `wideband` | 高頻帶明顯跟著聲音出現 → **16 kHz 寬頻**，ASR 可用 |
| `no-hf-response` | 高頻帶不隨聲音變化 → 8 kHz 窄頻**或**你的語音沒有高頻（要發「ㄙ」才能區分） |
| `too-quiet` | 幾乎沒出聲 → **不予判定**（工具會拒答，這是故意的） |
| `low-level` | 有高頻反應但太小聲 → 可能只是雜音，不予判定 |

⚠️ **為什麼不能只看「高頻有能量」：** 極安靜的錄音在高頻帶充滿**量化雜訊**，
看起來也有能量。實測純底噪的 4–8k/0.3–3k 比值約 **-20 dB**，與正常語音同級
→ 這個比值**單獨看沒有判別力**，會把 8 kHz 窄頻誤判成 16 kHz 寬頻。
正確做法是看高頻帶**會不會跟著聲音變大**。

⚠️ **為什麼不能只看 `--probe`：** Windows 藍牙音訊堆疊會把 8 kHz 窄頻連結
重採樣成你要求的任何取樣率。要 48 kHz 它也「接受」，但 4 kHz 以上全是空的。
...
**動到 `estimate_bandwidth` 就必須先跑這個測試** —— 這個判定會決定
「中文準確率 ≥ 95%」的門檻能不能成立，判錯會把不合格的硬體判成合格。
