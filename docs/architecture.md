...
| 項目 | 內容 |
|---|---|
| 輸入 | 使用者綁定的**任意鍵碼**（見 AGENTS.md §8） |
| 輸出 | `press` / `release` 事件 |
| 鐵則 1 | **不得硬編 F9**；必須支援一般鍵、consumer control、自訂 collection |
| 鐵則 2 | **必須「吃掉」按鍵**，不讓它傳到前景視窗（見下方 §3.1） |
| 鐵則 3 | **必須能分辨裝置**，只抑制目標裝置的按鍵，不影響使用者的實體鍵盤 |

#### 3.1 為什麼要抑制？（P0 實測結論）

裝置的 6 顆按鈕送的是 **Enter / Backspace / 上下左右**（`hardware.md` §3.3）。
這些是**標準鍵盤事件**，Windows 會**同時**把它們送到前景視窗。若不抑制：

| 按鈕 | 使用者會看到的副作用 |
|---|---|
| Enter | 目標輸入框送出表單／插入換行 |
| Backspace | **刪掉游標前一個字** |
| 方向鍵 | 游標亂跑／叫出歷史紀錄 |

**這會直接破壞產品體驗**，所以抑制是必要條件，不是加分項。

#### 3.2 實作限制（三棧都一樣，沒有現成解法）

| 機制 | 可抑制 | 可辨識裝置 |
|---|---|---|
| Raw Input | ❌ 只能觀察 | ✅ 有 `hDevice` |
| `global-hotkey`（Rust） | ❌ | ❌ |
| `WH_KEYBOARD_LL` 回傳非 0 | ✅ | ❌ **`KBDLLHOOKSTRUCT` 沒有裝置欄位** |
| `pynput` `suppress=True`（Python） | ✅ | ❌ |

**必要設計：兩條通道做時間關聯。**
低階 hook 負責「抑制」，Raw Input 負責「辨識裝置」；
靠時間戳把兩邊配對，才能決定「這顆鍵是不是目標裝置送的、要不要吃掉」。

這是自製元件，**Rust / Python / Go 三棧都沒有現成套件可直接用** ——
`plan.md` §6 評分表的「全域熱鍵」25% 必須照這個標準評分。
